...
jQuery.fn.colourPicker = function (conf) {
	...
	var config = jQuery.extend({
		...
		title:		'Выбери цвет',		// Default dialogue title
	...
	return this.each(function () {
		...
		var icon	= jQuery('<a href="#"><img style="width: 18px;" src="' + config.ico + '" alt="' + config.openTxt + '" /></a>').insertAfter(select);
		...
		if (config.inputBG) {
			input.change(function () {
				input.css({border: "1px solid rgb(12,71,127)",background: '#' + input.val(), color: '#' + hexInvert(input.val())});
			});
		...
		icon.click(function () {
			...
			jQuery('a', colourPicker).click(function () {
				...
				// If user wants to, change the input's BG to reflect the newly selected colour
				if (config.inputBG) {
					input.css({background: '#' + hex, color: '#' + hexInvert(hex), border: "1px solid rgb(12,71,127)"});
				}
